...
extern int unzipDVDFile( unsigned char *outbuffer, u64 discoffset, unsigned int length);
...
int updateSDdirname()
{
  ...
  char temp[1024];
   ...
   else if (strcmp(filelist[selection].filename,"..") == 0) 
   {
	 ...
	 /* handles root name */
	 if (strcmp(rootSDdir,"dev0:") == 0) sprintf(rootSDdir,"dev0:\\hugo\\..");
   ...
   else
   {
     ...
     {
       /* handles root name */
	   if (strcmp(rootSDdir,"dev0:\\hugo\\..") == 0) sprintf(rootSDdir,"dev0:");
	...
	while (haverom == 0)
	{
		...
		if (p & PAD_BUTTON_B)
		{
			...
			if (((!UseSDCARD) && (basedir == rootdir)) ||
				(UseSDCARD && strcmp(rootSDdir,"dev0:\\hugo\\..") == 0)) return;
			go_up = 1;
		}
		...
		if ((p & PAD_BUTTON_A) || go_up)
		{
			...
			else /*** This is a file ***/
			{
				rootdir = filelist[selection].offset;
				rootdirlength = filelist[selection].length;
              	hugoromsize = LoadFile(&hugorom[0]);
			  	cart_reload = 1;
			  	haverom = 1;
	      	}
		  	redraw = 1;
...
int OpenSD () 
{
  ...
  if (haveSDdir == 0)
  {
     ...
     sprintf(rootSDdir,"dev0:\\hugo\\roms");
	 ...
	 {
		/* no entries found */
		WaitPrompt ("Error reading dev0:\\hugo\\roms");
		return 0;
	 }
  }
...
int LoadFile (unsigned char *buffer) 
{
  ...
  else return unzipDVDFile (buffer, discoffset, rootdirlength);
