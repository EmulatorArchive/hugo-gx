...
void write_font(int x, int y, unsigned char *string)
{
	int ox = x;
...
void write_centre( int y, unsigned char *string )
{
	int x, t;

	for (x = t = 0; t < strlen((char *)string); t++) x += font_size[string[t]];
	if (x>back_framewidth) x=back_framewidth;
	x = (640 - x) >> 1;
	write_font(x, y, string);
}

void writex(int x, int y, int sx, int sy, unsigned char *string, unsigned int *lookup)
{
	int ox = x;
...
void write_centre_hi(int y, unsigned char *string)
{
	int x,t,h;
    for (x = t = 0; t < strlen((char *)string); t++) x += font_size[string[t]];
	if (x>back_framewidth) x = back_framewidth;
