...
int saveSettings() {
	...
	createXMLSetting("ip", "Share Computer IP", HugoSettings.ip);
	createXMLSetting("share", "Share Name", HugoSettings.share);
	createXMLSetting("user", "Share Username", HugoSettings.user);
	createXMLSetting("pwd", "Share Password", HugoSettings.pwd);
			
	FILE *file;
	file = fopen("sd:/hugo/hugo.xml", "wb");
 ...
 * Load Settings from XML File. 
 ToDo
 Check File auslagern!
 ***************************************************************************/ 
 
int load_settings() 
{
	FILE *fp = fopen("sd:/hugo/hugo.xml", "rb");
	...
	else 
	{
		...
		if (settings_size > 0) 
		{
			...
			if(item)
			{
				sprintf(HugoSettings.ip, "%s", mxmlElementGetAttr(item,"value"));
			}
			
			item = mxmlFindElement(xml, xml, "setting", "name", "share", MXML_DESCEND);
			if(item)
			{
				sprintf(HugoSettings.share, "%s", mxmlElementGetAttr(item,"value"));
			}
			
			item = mxmlFindElement(xml, xml, "setting", "name", "user", MXML_DESCEND);
			if(item)
			{
				sprintf(HugoSettings.user, "%s", mxmlElementGetAttr(item,"value"));
			}
			
			item = mxmlFindElement(xml, xml, "setting", "name", "pwd", MXML_DESCEND);
			if(item)
			{
				sprintf(HugoSettings.pwd, "%s", mxmlElementGetAttr(item,"value"));
			}
